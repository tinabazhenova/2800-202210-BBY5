...
app.get("/profile", function (req, res) {
  ...
  if (req.session.loggedIn) {
    ...
    connection.query(
      `SELECT * FROM ${userTable} WHERE ${userTable}.first_name = '${req.session.username}'`,
...
function wrap(filename, session) {
  ...
  dom.window.document.getElementById("templateContent").innerHTML =
    ...
    connection.query(
      ...
      function (error, results) {
        ...
        mainDOM.window.document.getElementsByTagName("title")[0].innerHTML =
          req.session.username + "'s Admin Page";
        mainDOM.window.document.getElementById("profile_name").innerHTML =
          "Welcome Admin " + req.session.username;
...
app.post("/login", function (req, res) {
  ...
  connection.query(
    ` SELECT * FROM ${userTable} WHERE user_name = "${req.body.username}" AND password = "${req.body.password}" `,
    function (error, results) {
...
app.get("/profile", function (req, res) {
  ...
  if (req.session.loggedIn) {
    ...
    profileDOM.window.document.getElementById("picture_src").src =
      req.session.userImage;
...
app.post("/update-username", function (req, res) {
  ...
  connection.query(
    `UPDATE ${userTable} SET user_name = ? WHERE ID = ?`,
...
app.get("/get-username", function (req, res) {
  res.setHeader("Content-Type", "application/json");
  connection.query(
    `SELECT user_name FROM ${userTable} WHERE ID = ? `,
    [req.session.userID],
...
app.post("/add-user", function (req, res) {
  res.setHeader("Content-Type", "application/json");
  connection.query(
    `INSERT INTO ${userTable} (user_name, first_name, last_name, password, is_admin) values (?, ?, ?, ?, ?)`,
    [
      req.body.user_name,
      req.body.first_name,
      req.body.last_name,
...
app.post("/edit-user", function (req, res) {
  res.setHeader("Content-Type", "application/json");
  connection.query(
    `UPDATE ${userTable} SET user_name = ?, first_name = ?, last_name = ?, password = ?, is_admin = ? WHERE ID = ?`,
    [
      req.body.user_name,
      req.body.first_name,
      req.body.last_name,
...
app.post("/delete-users", function (req, res) {
  res.setHeader("Content-Type", "application/json");
  connection.query(
    `DELETE FROM ${userTable} WHERE (user_name) = ? `,
    [req.body.user_name],
    function (error, results, fields) {
      if (error) {
        console.log(error);
